...
describe(fileShortPath(import.meta.url), () => {
  ...
  it(`QueueManager.getMetrics(${rndString})`, async () => {
  ...
  it(`QueueManager.getMetrics(${rndString}) after send()`, async () => {
  ...
  it(`QueueManager.getMetrics(FAKE)`, async () => {
    const row = await mq.queue.getMetrics(genRandomString(7))
    assert(row === null, 'getMetrics should return null')
  })
